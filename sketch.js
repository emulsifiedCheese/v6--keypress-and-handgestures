...
function nameInfo() {
  ...
  info.forEach(([text, y]) => {
    createLabel(text, vidW * 3.5, y, "18px");
  });
}

function instructions() {
  //create labels for instructions and active effects
  createLabel("Instructions:", vidW * 3.5, 80, "18px");
  ...
  instructions.forEach((text, i) => {
    createLabel(text, vidW * 3.5, 110 + i * 20, "16px");
  });
  ...
  indicators.forEach((text, i) => {
    createLabel(text, vidW * 3.5, 260 + i * 20, "16px");
...
function updateFilterStatus() {
  ...
  effects.forEach((isActive, i) => {
    filterStatusLabels[i].html(isActive ? "Applied" : "Not Applied"); //update filter status label
